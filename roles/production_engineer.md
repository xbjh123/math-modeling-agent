...
3. **数值溯源防幻觉**：**强制要求**——每个准备进入论文的数值（结果、参数、指标、图例标注）都必须写入 `02_execution_log.json`，记录：数值、来源代码文件、输入切片、运行时间戳。凡无法在 log 中溯源的数字一律不得进入论文。**推导常量同样入 log（2024A 教训）**：由题面输入换算/推导出的常数（如把手孔距 = 板长 − 2×孔边距）虽非直接求解输出，但会出现在论文中，必须以 `derived_constants` 字段写入 log，含推导公式与来源。
